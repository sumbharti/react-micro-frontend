...
const Remote = () => {
    ...
    useEffect(() => {
        ...
        appInsights.trackEvent({ name: "RemoteComponent_Loaded", properties: { timestamp: new Date().toISOString() } });
        appInsights.trackPageView({ name: "RemoteComponent", uri: "/remote" });
        appInsights.trackTrace({ message: "Remote component loaded", severityLevel: 1 });
        
        appInsights.trackTrace({ message: 'Loading contacts from dataverse' });
        ...
        appInsights.trackMetric({
            name: `Component.Remote.RenderTime`,
            average: duration,
        });
        
        if (duration > 100) {
        appInsights.trackTrace({
            message: `Slow component render: Remote took ${duration.toFixed(0)}ms`,
